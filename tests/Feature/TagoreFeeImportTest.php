...
class TagoreFeeImportTest extends TestCase
{
    ...
    {
        $controller=file_get_contents(base_path('app/Http/Controllers/Tagore/AdministrationController.php')); $view=file_get_contents(base_path('resources/views/tagore/administration.blade.php'));
    ...
    public function test_legacy_import_is_mapping_safe_and_ledger_reconciliation_only(): void
    {
        ...
        foreach(['ALL LEDGER','ledger_reference','explicit legacy-student mapping','one_time_amount','tagore_fee_concessions'] as $needle) $this->assertStringContainsString($needle,$service);
        ...
        foreach(['report(','tagore_fee_obligations','outstanding_amount','legacy_balance','mismatch'] as $needle) $this->assertStringContainsString($needle,$recon);
    }
