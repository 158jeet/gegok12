...
class FeeManagementController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $structures = DB::table('tagore_fee_structures as f')->join('tagore_institutions as i','i.id','=','f.institution_id')->leftJoin('academic_years as ay','ay.id','=','f.academic_year_id')->whereIn('f.institution_id',$ids)->orderByDesc('f.id')->get(['f.*','i.display_name as institution','ay.name as academic_year']);
        $students = DB::table('users as u')->whereIn('u.school_id',$institutions->pluck('school_id'))->where('u.usergroup_id',6)->orderBy('u.name')->limit(500)->get(['u.id','u.name','u.school_id']);
    ...
    {
        $userId=(int)$request->user()->id; $roles=$this->roles($userId); abort_unless($roles->intersect(['OWNER','PRINCIPAL','COORDINATOR','ACCOUNTS'])->isNotEmpty(),403);
        $data=$request->validate(['institution_id'=>['required','integer','exists:tagore_institutions,id'],'academic_year_id'=>['nullable','integer','exists:academic_years,id'],'name'=>['required','string','max:160'],'description'=>['nullable','string','max:1000'],'frequency'=>['required','in:annual,term,monthly,one_time'],'heads'=>['required','array','min:1'],'heads.*.name'=>['required','string','max:120'],'heads.*.code'=>['required','string','max:60'],'heads.*.amount'=>['required','numeric','min:0'],'heads.*.category'=>['nullable','string','max:60'],'heads.*.optional'=>['nullable','boolean']]);
        $this->authorizeInstitution($userId,(int)$data['institution_id'],$roles);
        DB::transaction(function() use($data){$structureId=DB::table('tagore_fee_structures')->insertGetId(['institution_id'=>$data['institution_id'],'academic_year_id'=>$data['academic_year_id']??null,'name'=>$data['name'],'description'=>$data['description']??null,'frequency'=>$data['frequency'],'status'=>'active','created_at'=>now(),'updated_at'=>now()]); foreach($data['heads'] as $head){DB::table('tagore_fee_components')->insert(['fee_structure_id'=>$structureId,'name'=>$head['name'],'code'=>$head['code'],'category'=>$head['category']??null,'amount'=>round((float)$head['amount'],2),'tax_rate'=>0,'is_optional'=>(bool)($head['optional']??false),'status'=>'active','created_at'=>now(),'updated_at'=>now()]);}});
    ...
    public function generateDemand(Request $request, FeeService $feeService)
    {
        ...
        $structure=DB::table('tagore_fee_structures')->where('id',$data['fee_structure_id'])->first(); abort_unless($structure,404); $this->authorizeInstitution($userId,(int)$structure->institution_id,$roles);
        ...
        $id=$feeService->createObligation((int)$student->id,(int)$structure->institution_id,['academic_year_id'=>$structure->academic_year_id,'fee_structure_id'=>$structure->id,'due_date'=>$data['due_date']??null,'items'=>$items,'gross_amount'=>$total,'discount_amount'=>$discount,'concession_amount'=>$concession],$userId);
        $this->createInstallments($id,$total,$structure->frequency,$data['due_date']??null);
        return back()->with('success',"Student demand #{$id} created with installments.");
    }

    private function createInstallments(int $obligationId,float $total,string $frequency,?string $dueDate):void
    {
        $count=$frequency==='monthly'?12:($frequency==='term'?3:1); $base=round($total/$count,2); $running=0; $start=$dueDate?\Carbon\Carbon::parse($dueDate):now();
        for($n=1;$n<=$count;$n++){ $amount=$n===$count?round($total-$running,2):$base; $running=round($running+$amount,2); $date=$count===1?$start->copy():$start->copy()->addMonths($n-1); DB::table('tagore_fee_installments')->insert(['fee_obligation_id'=>$obligationId,'installment_no'=>$n,'name'=>$count===1?'Full Fee':($frequency==='monthly'?"Month {$n}":"Term {$n}"),'due_date'=>$date->toDateString(),'amount'=>$amount,'paid_amount'=>0,'outstanding_amount'=>$amount,'status'=>'pending','created_at'=>now(),'updated_at'=>now()]); }
    }
