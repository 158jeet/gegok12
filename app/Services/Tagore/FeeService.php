...
{
    /**
     * Create a student fee obligation and its line items.
     * Amounts are stored as the source-of-truth ledger values; no implicit
     * class-level assumptions are made here.
     */
     ...
     * Allocation is mandatory for ledger integrity when obligations exist.
     */
    public function recordOfflinePayment(
        ...
        return DB::transaction(function () use ($studentId, $institutionId, $amount, $parentUserId, $createdBy, $reference, $allocations, $paymentMode) {
            ...
            if ($allocations && abs($requested - $amount) > 0.009) {
            ...
            $paymentId = DB::table('tagore_payments')->insertGetId([
                ...
                'status' => 'success',
                'paid_at' => $now,
                'notes' => $reference ?: 'Offline payment',
                'created_at' => $now,
                'updated_at' => $now,
            ]);

            if (!$allocations) {
                $open = DB::table('tagore_fee_obligations')
                ...
                if ($remaining > 0.009) {
                    throw ValidationException::withMessages(['amount' => 'Payment exceeds the student outstanding balance.']);
                }
            }
            ...
            foreach ($allocations as $allocation) {
                ...
                if (!$obligation || $obligation->student_id !== $studentId || $obligation->institution_id !== $institutionId) {
                    throw ValidationException::withMessages(['allocations' => 'Invalid fee obligation allocation.']);
                }
                if ($allocate <= 0 || $allocate > ((float) $obligation->outstanding_amount + 0.009)) {
                    throw ValidationException::withMessages(['allocations' => 'Allocation exceeds the obligation outstanding amount.']);
                }
                ...
                DB::table('tagore_fee_obligations')->where('id', $obligationId)->update([
                    'paid_amount' => $paid,
                ...
                if (!empty($allocation['fee_installment_id'])) {
                    $this->refreshInstallment((int) $allocation['fee_installment_id']);
                }
            }
            ...
            $this->audit($studentId, $institutionId, 'FEE_PAYMENT_RECORDED', 'tagore_payments', $paymentId, $createdBy, [
                'amount' => $amount,
                'receipt_no' => $receiptNo,
                'payment_mode' => $paymentMode,
            ]);
    ...
    private function refreshInstallment(int $installmentId): void
    {
        ...
        $paid = round((float) DB::table('tagore_payment_allocations')
            ->where('fee_installment_id', $installmentId)
            ->sum('amount'), 2);
        $outstanding = max(0, round((float) $installment->amount - $paid, 2));

        DB::table('tagore_fee_installments')->where('id', $installmentId)->update([
            'paid_amount' => $paid,
