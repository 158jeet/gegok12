...
class LegacyFeeMigrationService
{
    public function preview(string $path, int $institutionId, ?int $academicYearId, int $actorId): array
    {
        ...
        foreach($book->getWorksheetIterator() as $sheet){
            $type=$this->sheetType($sheet->getTitle()); if(!$type || $type==='students'||$type==='ledger') continue;
        ...
        foreach($items as $item){
            ...
            if(in_array($type,['student_fee','concession','opening_balance'],true)&&!$student) $error=$matchError;
    ...
    private function sheetType(string $title):?string
    {
        $t=strtoupper(trim($title)); return match($t){
            ...
            'XII SCI FEE STRUCTURE','XII SCIENCE FEE STRUCTURE'=>'student_fee','FEE CONCESSION','FEE CONCESSIONS'=>'concession','STUDENTS'=>'students','ALL LEDGER'=>'ledger',default=>null};
    }
    ...
    private function headerHints(string $type):array{return match($type){'fee_structure'=>['CLASS','STANDARD','CLASS NAME','STD','TOTAL','TOTAL FEE','ONE TIME','ONE TIME FEE'],'opening_balance'=>['STUDENT','STUDENT NAME','FEE AMOUNT','AMOUNT RECEIVED','BALANCE'],'student_fee'=>['STUDENT','STUDENT NAME','RECEIPT NO','RECEIPT NUMBER','TOTAL','AMOUNT','RECEIVED'],'concession'=>['STUDENT','STUDENT NAME','CONCESSION','DISCOUNT'],'transport_route'=>['ROUTE','ROUTE NAME','BUS ROUTE','STOP','STOP NAME','FEE','AMOUNT'],default=>[]};}
