...
{
    public function __construct(private LegacyFeeWorkbookParser $workbookParser) {}
    ...
    private function studentFee($batch,$r,$row,$actor,&$c):void{
        $g=$this->money($this->v($row,['TOTAL','TOTAL FEE','GROSS','FEE AMOUNT','AMOUNT','PAYABLE']));
        $d=$this->money($this->v($row,['DISCOUNT','DISCOUNT AMOUNT']));
        $co=$this->money($this->v($row,['CONCESSION','CONCESSION AMOUNT']));
        $payments=$this->legacyPayments($row);
        $p=round(array_sum(array_column($payments,'amount')),2);
        if($p<=0)$p=$this->money($this->v($row,['RECEIVED','RECEIVED AMOUNT','PAID','AMOUNT RECEIVED']));
        $net=max(0,round($g-$d-$co,2));
        if(!$r->student_id||($g<=0&&$p<=0))return;
        $exists=DB::table('tagore_fee_obligations')->where('student_id',$r->student_id)->where('academic_year_id',$batch->academic_year_id)->where('status','legacy_import')->where('gross_amount',$g)->where('paid_amount',$p)->exists();
        if($exists)return;
        $id=DB::table('tagore_fee_obligations')->insertGetId(['student_id'=>$r->student_id,'institution_id'=>$batch->institution_id,'academic_year_id'=>$batch->academic_year_id,'fee_structure_id'=>null,'due_date'=>$this->date($this->v($row,['DUE DATE','DATE'])),'gross_amount'=>$g,'discount_amount'=>$d,'concession_amount'=>$co,'net_amount'=>$net,'paid_amount'=>$p,'outstanding_amount'=>max(0,$net-$p),'status'=>$p>=$net&&$net>0?'paid':'legacy_import','created_at'=>now(),'updated_at'=>now()]);
        $item=DB::table('tagore_fee_obligation_items')->insertGetId(['fee_obligation_id'=>$id,'fee_head'=>'Legacy 2026-27 Fee','code'=>'LEGACY_2026_27','gross_amount'=>$g,'discount_amount'=>$d,'concession_amount'=>$co,'net_amount'=>$net,'paid_amount'=>$p,'outstanding_amount'=>max(0,$net-$p),'metadata_json'=>json_encode(['import_batch_id'=>$batch->id,'source_hash'=>$r->source_hash]),'created_at'=>now(),'updated_at'=>now()]);
        foreach($payments as $paymentData){
            $amount=round((float)$paymentData['amount'],2); if($amount<=0)continue;
            $receipt=$paymentData['receipt']??$this->v($row,['RECEIPT NO','RECEIPT NUMBER','RECEIPT']);
            $payment=DB::table('tagore_payments')->insertGetId(['payment_order_id'=>null,'student_id'=>$r->student_id,'parent_user_id'=>null,'institution_id'=>$batch->institution_id,'amount'=>$amount,'currency'=>'INR','gateway'=>null,'gateway_order_id'=>null,'gateway_payment_id'=>null,'status'=>'success','paid_at'=>$this->date($paymentData['date']??null)?:$this->date($this->v($row,['DATE','PAYMENT DATE','RECEIPT DATE']))?:now(),'receipt_no'=>$receipt,'payment_mode'=>'legacy','reference_number'=>$receipt,'notes'=>'Imported legacy fee payment','created_at'=>now(),'updated_at'=>now()]);
            DB::table('tagore_payment_allocations')->insert(['payment_id'=>$payment,'fee_obligation_id'=>$id,'fee_installment_id'=>null,'fee_obligation_item_id'=>$item,'amount'=>$amount,'created_at'=>now(),'updated_at'=>now()]);
            DB::table('tagore_financial_transactions')->insert(['institution_id'=>$batch->institution_id,'student_id'=>$r->student_id,'transaction_type'=>'PAYMENT','reference_type'=>'tagore_payment','reference_id'=>$payment,'debit'=>0,'credit'=>$amount,'description'=>'Imported legacy fee payment','transaction_date'=>$this->date($paymentData['date']??null)?:now(),'created_by'=>$actor,'created_at'=>now(),'updated_at'=>now()]);
            $c['payments']++;
        }
        if(!$payments&&$p>0){
            $payment=DB::table('tagore_payments')->insertGetId(['payment_order_id'=>null,'student_id'=>$r->student_id,'parent_user_id'=>null,'institution_id'=>$batch->institution_id,'amount'=>$p,'currency'=>'INR','gateway'=>null,'gateway_order_id'=>null,'gateway_payment_id'=>null,'status'=>'success','paid_at'=>$this->date($this->v($row,['DATE','PAYMENT DATE','RECEIPT DATE']))?:now(),'receipt_no'=>$this->v($row,['RECEIPT NO','RECEIPT NUMBER','RECEIPT']),'payment_mode'=>'legacy','reference_number'=>$this->v($row,['RECEIPT NO','RECEIPT NUMBER','RECEIPT']),'notes'=>'Imported legacy fee payment','created_at'=>now(),'updated_at'=>now()]);
            DB::table('tagore_payment_allocations')->insert(['payment_id'=>$payment,'fee_obligation_id'=>$id,'fee_installment_id'=>null,'fee_obligation_item_id'=>$item,'amount'=>$p,'created_at'=>now(),'updated_at'=>now()]);
            DB::table('tagore_financial_transactions')->insert(['institution_id'=>$batch->institution_id,'student_id'=>$r->student_id,'transaction_type'=>'PAYMENT','reference_type'=>'tagore_payment','reference_id'=>$payment,'debit'=>0,'credit'=>$p,'description'=>'Imported legacy fee payment','transaction_date'=>$this->date($this->v($row,['DATE','PAYMENT DATE','RECEIPT DATE']))?:now(),'created_by'=>$actor,'created_at'=>now(),'updated_at'=>now()]);
            $c['payments']++;
        }
        $c['student_fees']++;
    }

    private function legacyPayments(array $row):array
    {
        $payments=$row['PAYMENTS']??[];
        if(is_array($payments)&&$payments)return array_values(array_filter($payments,fn($p)=>is_array($p)&&$this->money($p['amount']??0)>0));
        return [];
    }
    private function concession($batch,$r,$row,$actor,&$c):void{$a=$this->money($this->v($row,['CONCESSION','CONCESSION AMOUNT','DISCOUNT','DISCOUNT AMOUNT']));if(!$r->student_id||$a<=0)return;DB::table('tagore_fee_concessions')->updateOrInsert(['student_id'=>$r->student_id,'institution_id'=>$batch->institution_id,'fee_obligation_id'=>null,'name'=>'Legacy concession','amount'=>$a],['fee_component_id'=>null,'concession_type'=>'fixed','percentage'=>null,'reason'=>'Legacy import: '.trim((string)$this->v($row,['REASON','REMARK','REMARKS','CONCESSION REASON'])),'approved_by'=>$actor,'approved_at'=>now(),'status'=>'approved','updated_at'=>now()]);$c['concessions']++;}
    ...
    private function studentKey(array $row):?string{$v=$this->v($row,['SR NO','SRNO','REG NO','REGISTRATION NO','REGISTRATION NUMBER','STUDENT ID','ADM NO','ADMISSION NO','ADMISSION NUMBER']);return$v===null?null:trim((string)$v);}
