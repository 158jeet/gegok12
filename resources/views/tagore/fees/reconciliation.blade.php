...
@foreach([['Ledger rows',$report['total_rows'],''],['Matched',$report['matched'],'text-success'],['Mismatches',$report['mismatch'],'text-danger'],['Net delta','₹'.number_format($report['totals']['difference'],2),'']] as $card)
...
@forelse($report['items'] as $item)<tr><td>{{ $item['row'] }}</td><td>{{ $item['student_name'] ?: 'Student #'.$item['student_id'] }}</td><td>₹{{ number_format($item['legacy_balance'],2) }}</td><td>₹{{ number_format($item['tagore_balance'],2) }}</td><td class="{{ abs($item['difference'])>0.01?'text-danger fw-bold':'' }}">₹{{ number_format($item['delta'],2) }}</td><td>{{ $item['status'] }}</td></tr>@empty<tr><td colspan="6">No mapped ALL LEDGER rows found.</td></tr>@endforelse
