...
@forelse($batches as $b)<tr><td>{{ $b->id }}</td><td>{{ $b->institution }}</td><td>{{ $b->source_name }}</td><td><span class="badge bg-secondary">{{ $b->status }}</span></td><td>{{ $b->row_count }}</td><td>{{ $b->success_count }}</td><td class="{{ $b->error_count ? 'text-danger fw-bold' : '' }}">{{ $b->error_count }}</td><td>@if($b->status==='needs_review')<a class="btn btn-sm btn-outline-primary me-1" href="{{ route('tagore.fees.import.mapping.suggestions',$b->id) }}">Match students</a><a class="btn btn-sm btn-warning" href="{{ route('tagore.fees.import.mapping',$b->id) }}">Manual mapping</a>@elseif($b->status==='ready')<a class="btn btn-sm btn-outline-primary me-1" href="{{ route('tagore.fees.import.reconciliation',$b->id) }}">Reconciliation</a><form method="POST" action="{{ route('tagore.fees.import.apply',$b->id) }}" class="d-inline">@csrf<button class="btn btn-sm btn-success">Apply validated batch</button></form>@elseif($b->status==='applied')<a class="btn btn-sm btn-outline-primary" href="{{ route('tagore.fees.import.reconciliation',$b->id) }}">Reconciliation</a>@else<span class="text-muted">Review required</span>@endif</td></tr>@empty<tr><td colspan="8">No migration batches yet.</td></tr>@endforelse
