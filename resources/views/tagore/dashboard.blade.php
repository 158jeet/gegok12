...
@if($managerCommand['is_manager'])<div class="section card"><h2>Manager Command Center</h2><div class="grid" style="margin:0 0 14px"><div class="card"><div class="label">Active staff</div><div class="value">{{ $managerCommand['active_staff'] }}</div></div><div class="card"><div class="label">Open work</div><div class="value">{{ $stats['open_tasks'] }}</div></div><div class="card"><div class="label">Overdue</div><div class="value">{{ $stats['overdue_tasks'] }}</div></div><div class="card"><div class="label">Completion rate</div><div class="value">{{ $managerCommand['completion_rate'] }}%</div></div></div><h3>Needs attention</h3>@if($managerCommand['action_items']->isEmpty())<div class="muted">No urgent work requires attention.</div>@else<table class="table"><tr><th>Task</th><th>Department</th><th>Owner</th><th>Status</th><th>Due</th></tr>@foreach($managerCommand['action_items'] as $item)<tr><td><a href="{{ route('tagore.tasks.index', ['assigned_to' => $item->assignee ? null : 'unassigned']) }}">{{ $item->title }}</a></td><td>{{ $item->department ?: '—' }}</td><td>{{ $item->assignee ?: 'Unassigned' }}</td><td>{{ ucfirst(str_replace('_',' ',$item->status)) }}</td><td>{{ $item->due_at ? date('d M H:i', strtotime($item->due_at)) : 'No due date' }}</td></tr>@endforeach</table>@endif<h3 style="margin-top:18px">Department health</h3><table class="table"><tr><th>Department</th><th>Active</th><th>Overdue</th><th>Blocked</th></tr>@foreach($managerCommand['departments'] as $department)<tr><td>{{ $department->name }}</td><td>{{ $department->active }}</td><td>{{ $department->overdue }}</td><td>{{ $department->blocked }}</td></tr>@endforeach</table></div>@endif
